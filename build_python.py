...
def install_pyenv_version(version):
    ...
    if linux():
        ...
        if centos():
            ...
            subprocess.run("sed -i 's#\"${!PACKAGE_CONFIGURE_OPTS_ARRAY}\" $CONFIGURE_OPTS ${!PACKAGE_CONFIGURE_OPTS} || return 1#\"${!PACKAGE_CONFIGURE_OPTS_ARRAY}\" $CONFIGURE_OPTS --enable-shared LD_RUN_PATH=/opt/ccdc/third-party/base_python/base_python-3.11.6-20240105.2-centos7/lib LDFLAGS=\"-L/opt/ccdc/third-party/base_python/base_python-3.11.6-20240105.2-centos7/lib -lsqlite3 -L/usr/lib64/openssl11 -lssl -lcrypto\" CPPFLAGS=\"-I/opt/ccdc/third-party/base_python/base_python-3.11.6-20240105.2-centos7/include -I/usr/include/openssl11\" ${!PACKAGE_CONFIGURE_OPTS} || return 1#' /tmp/pyenvinst/plugins/python-build/bin/python-build", shell=True, check=True, env=python_build_env)
